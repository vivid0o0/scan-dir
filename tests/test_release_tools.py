...
VERIFY_SPEC = importlib.util.spec_from_file_location("scan_dir_verify_release", VERIFY_RELEASE)
...
def test_release_reader_consumes_complete_regular_file_without_size_ceiling(tmp_path: Path) -> None:
    source = tmp_path / "large-release-input"
    payload = (b"0123456789abcdef" * (1024 * 1024 + 1)) + b"tail"
    source.write_bytes(payload)
    assert verify_release.read_regular_file(source) == payload


def test_validation_configuration_has_no_partial_or_clock_bounded_success() -> None:
    workflow = (ROOT / ".github" / "workflows" / "ci.yml").read_text(encoding="utf-8")
    assert "timeout-minutes:" not in workflow
    assert "cancel-in-progress:" not in workflow

    project = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert "fail_under = 100" in project
    assert "skip_covered = false" in project


def test_test_harness_process_waits_have_no_deadline_keyword() -> None:
    process_wait_calls = {"run", "check_call", "check_output", "communicate", "wait"}
    for source in sorted((ROOT / "tests").glob("test_*.py")):
        tree = ast.parse(source.read_text(encoding="utf-8"), filename=str(source))
        for call in ast.walk(tree):
            if not isinstance(call, ast.Call) or not isinstance(call.func, ast.Attribute):
                continue
            if call.func.attr not in process_wait_calls:
                continue
            assert all(keyword.arg != "timeout" for keyword in call.keywords), (
                source,
...
def test_release_builder_rejects_concurrent_output_writer(tmp_path: Path) -> None:
    ...
    try:
        ...
        holder_stdout, holder_stderr = holder.communicate("release\n")
