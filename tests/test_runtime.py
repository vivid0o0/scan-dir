...
spec = importlib.util.spec_from_file_location("project_summarizer_runtime", PRS)
...
def run_prs(*args: str, cwd: Path | None = None, env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
...
def test_no_site_runtime_smoke() -> None:
    result = run_prs_subprocess("version")
    assert result.returncode == 0
    assert result.stdout.strip() == f"prs {prs.VERSION}"
    assert result.stderr == ""


def test_commands_and_invalid_attached_flags(tmp_path: Path) -> None:
    assert run_prs("version").stdout.strip() == f"prs {prs.VERSION}"
    assert "Project Summarizer" in run_prs("help").stdout
    assert run_prs("help", "extra").returncode == 2
    assert run_prs("--help=garbage").returncode == 2
    assert run_prs("--version=garbage").returncode == 2
    command_path = tmp_path / "help"
    command_path.mkdir()
    (command_path / "x.txt").write_text("x", encoding="utf-8")
    result = run_prs("./help", cwd=tmp_path)
    assert result.returncode == 0 and "x.txt" in result.stdout


def test_filtering_flat_and_summary(tree: Path) -> None:
    only = run_prs(str(tree), "--only", "-e", ".py", "--scan-styling", "minimal", "--scan-emojis", "false")
    ...
    flat = run_prs(str(tree), "--scan-data", "lines,size,type", "--scan-styling", "minimal", "--scan-emojis", "false")
    assert flat.returncode == 0
    assert "src/a.py" in flat.stdout and "tests/a.py" in flat.stdout

    summary = run_prs(str(tree), "--scan-data", "summary", "--scan-styling", "minimal", "--scan-emojis", "false")
...
def test_visibility_symlinks_binary_and_special(tree: Path) -> None:
    ...
    fifo = tree / "pipe"
    os.mkfifo(fifo)
    special = run_prs(str(tree), "--scan-styling", "minimal", "--scan-emojis", "false")
...
def test_sanitizes_hostile_names(tmp_path: Path) -> None:
    ...
    result = run_prs(str(tmp_path), "--scan-styling", "minimal", "--scan-emojis", "false")
    assert result.returncode == 0
...
def test_config_layers_and_validation(tmp_path: Path) -> None:
    ...
    result = run_prs(str(tmp_path), "--config", str(explicit))
    assert result.returncode == 0 and "keep.py" not in result.stdout

    (tmp_path / ".prs.yaml").write_text("names: []\n", encoding="utf-8")
    conflict = run_prs(str(tmp_path))
    assert conflict.returncode == 2 and "multiple project configuration" in conflict.stderr

    invalid = run_prs(str(tmp_path), "--scan-timeout", "nan")
...
def test_git_markers_deleted_and_ignored_directory(tmp_path: Path) -> None:
    ...
    result = run_prs(str(tmp_path), "--full", "--scan-styling", "minimal", "--scan-emojis", "false")
    assert result.returncode == 0
...
def test_deleted_paths_respect_ignored_ancestor_name(tmp_path: Path) -> None:
    ...
    result = run_prs(str(tmp_path), "--ignore", "-n", "vendor", "--scan-styling", "minimal", "--scan-emojis", "false")
    assert result.returncode == 0
    assert "vendor/gone.txt" not in result.stdout


def test_rendered_lines_fit_narrow_terminal(tree: Path) -> None:
    result = run_prs(str(tree), "--scan-styling", "minimal", "--scan-emojis", "false", env={"COLUMNS": "16"})
    assert result.returncode == 0
    too_wide = [(line, prs.cell_width(line)) for line in result.stdout.splitlines() if prs.cell_width(line) > 16]
    assert not too_wide


def test_help_status_and_version_fit_narrow_terminal() -> None:
    for command in ("help", "status", "version"):
        result = run_prs(command, env={"COLUMNS": "16"})
        assert result.returncode == 0
        too_wide = [(line, prs.cell_width(line)) for line in result.stdout.splitlines() if prs.cell_width(line) > 16]
        assert not too_wide
...
def test_headless_auto_copy_is_nonfatal(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-S", str(PRS), str(tmp_path), "--scan-data", "tree", "--auto-copy", "true"],
        env=env,
        text=True,
        capture_output=True,
        timeout=10,
...
def test_narrow_terminal_preserves_requested_metadata(tree: Path) -> None:
    ...
    assert all(prs.cell_width(line) <= 16 for line in result.stdout.splitlines())
...
def test_project_config_can_be_ignored_and_cannot_enable_auto_copy(tmp_path: Path) -> None:
    ...
    missing.mkdir()
    required = run_prs(str(missing), "--project-config", "require")
