...
def test_minimal_styling_preserves_literal_tree_glyph_sequences(tmp_path: Path) -> None:
    nested = tmp_path / "parent├── dir"
    nested.mkdir()
    literal_names = (
        "alpha├── beta.txt",
        "gamma└── delta.txt",
    ...
    for name, contents in zip(literal_names, ("x", "xx", "L" * 64), strict=True):
        (tmp_path / name).write_text(contents, encoding="utf-8")
    nested_leaf = nested / "leaf└── value.txt"
    nested_leaf.write_text("nested", encoding="utf-8")

    tree_result = run_sdir(
        str(tmp_path),
        "--scan-data",
        "max",
        "--scan-styling",
    ...
    for literal in (*literal_names, "parent├── dir", "leaf└── value.txt"):
        assert literal in tree_result.stdout
    for corrupted in ("alpha+-- beta.txt", "gamma`-- delta.txt", "largest|   value.txt"):
        assert corrupted not in tree_result.stdout
    assert "+-- " in tree_result.stdout and "`-- " in tree_result.stdout
    assert "largest largest│   value.txt" in tree_result.stdout

    flat_result = run_sdir(
        str(tmp_path),
        "--scan-data",
        "lines,size,type,summary",
        "--scan-styling",
        "minimal",
        "--scan-emojis",
        "false",
    ...
    assert "parent├── dir/leaf└── value.txt" in flat_result.stdout
    assert "parent+-- dir/leaf`-- value.txt" not in flat_result.stdout

    init_git(tmp_path)
    deleted = tmp_path / "gone├── value.txt"
    deleted.write_text("gone", encoding="utf-8")
    subprocess.run(["git", "add", "."], cwd=tmp_path, check=True)
    subprocess.run(["git", "commit", "-qm", "literal tree glyphs"], cwd=tmp_path, check=True)
    deleted.unlink()
    deleted_result = run_sdir(
        str(tmp_path),
        "--full",
        "--scan-data",
        "tree,git",
    ...
    assert "deleted gone├── value.txt [D]" in deleted_result.stdout
    assert "deleted gone+-- value.txt [D]" not in deleted_result.stdout

    fifo = tmp_path / "warn│   fifo"
    os.mkfifo(fifo)
    warning_result = run_sdir(
        str(tmp_path),
        "--scan-data",
        "summary",
        "--scan-styling",
    ...
    assert "warn│   fifo" in warning_result.stdout
    assert "warn|   fifo" not in warning_result.stdout
...
def test_narrow_terminal_preserves_root_and_long_filename(
    tmp_path: Path,
    styling: str,
    connector: str,
) -> None:
    root_name = "sdir-v13-root-name-that-must-not-be-truncated"
    file_name = "this-is-a-very-long-filename-that-must-not-be-truncated-界…txt"
    root = tmp_path / root_name
    root.mkdir()
    (root / file_name).write_text("x", encoding="utf-8")
    ...
    assert result.stdout == f"{root_name}/\n{connector}{file_name}\n\n"


def test_narrow_terminal_preserves_trailing_filename_spaces(tmp_path: Path) -> None:
    file_name = "trailing-space-name-that-must-remain  "
    ...
    assert result.returncode == 0, result.stderr
    assert result.stdout.splitlines()[1] == f"`-- {file_name}"


def test_inline_metadata_preserves_long_label(tmp_path: Path) -> None:
    file_name = "inline-metadata-label-that-must-remain-complete.txt"
    ...
    assert result.returncode == 0, result.stderr
    assert result.stdout.splitlines()[1] == f"`-- {file_name} file"


def test_narrow_terminal_preserves_deleted_git_path(tmp_path: Path) -> None:
    init_git(tmp_path)
    file_name = "deleted-path-that-must-not-be-truncated-界…txt"
    deleted = tmp_path / file_name
    deleted.write_text("gone", encoding="utf-8")
    subprocess.run(["git", "add", "."], cwd=tmp_path, check=True)
    subprocess.run(["git", "commit", "-qm", "deleted path identity"], cwd=tmp_path, check=True)
    deleted.unlink()
    ...
    assert result.returncode == 0, result.stderr
    assert result.stdout.splitlines().count(f"deleted {file_name} [D]") == 1


def test_narrow_summary_preserves_long_path_and_value(tmp_path: Path) -> None:
    file_name = "summary-path-that-must-not-be-truncated-界…txt"
    (tmp_path / file_name).write_text("x" * 64, encoding="utf-8")
    ...
    largest_start = lines.index("largest")
    newest_start = next(
        index for index, line in enumerate(lines[largest_start + 1 :], largest_start + 1) if line.startswith("newest")
    )
    assert "".join(lines[largest_start:newest_start]) == f"largest {file_name}    64 B"


def test_columns_above_500_preserve_long_flat_path_and_metadata(tmp_path: Path) -> None:
    parts = ("a" * 180, "b" * 180, "c" * 180)
    directory = tmp_path
    for part in parts:
        directory /= part
        directory.mkdir()
    file_name = "leaf-" + "d" * 100 + ".txt"
    (directory / file_name).write_text("x", encoding="utf-8")
    relative = "/".join((*parts, file_name))
    ...
    assert result.returncode == 0, result.stderr
    assert sdir.TERMINAL_WIDTH_MAXIMUM == 500
    name_width = sdir.TERMINAL_WIDTH_MAXIMUM - sdir.METADATA_COLUMN_WIDTH - sdir.cell_width(sdir.METADATA_SEPARATOR)
    expected = relative + " " * max(0, name_width - sdir.cell_width(relative)) + sdir.METADATA_SEPARATOR + "file"
    assert result.stdout.splitlines()[4] == expected
...
def test_narrow_terminal_preserves_fixture_labels_that_exceed_width(tree: Path) -> None:
    result = run_sdir(str(tree), "--scan-styling", "minimal", "--scan-emojis", "false", env={"COLUMNS": "16"})
    assert result.returncode == 0
    lines = result.stdout.splitlines()
    assert lines[0] == f"{tree.name}/"
    assert [line for line in lines if "broken-link" in line] == ["+-- broken-link -> missing"]
    assert [line for line in lines if "src-link" in line] == ["`-- src-link -> src"]
...
def test_requested_auto_copy_without_backend_returns_clipboard_failure(tmp_path: Path) -> None:
    ...
    assert "auto-copy failed" in result.stderr
...
def test_closed_downstream_pipe_exits_cleanly(tmp_path: Path) -> None:
    ...
    consumer_stderr = consumer.communicate()[1]
...
def test_narrow_terminal_preserves_requested_metadata(tree: Path) -> None:
    ...
    lines = result.stdout.splitlines()
    root_lines_index = next(index for index, line in enumerate(lines[1:], start=1) if line.startswith("  lines:"))
    assert "".join(line[2:] for line in lines[1:root_lines_index]) == "entries: 6 files, 3 dirs, 2 links"

    note_index = lines.index("+-- note.txt")
    assert lines[note_index + 1 : note_index + 4] == [
        "  entries: file",
        "  lines: 1L",
        "  size: 4 B",
    ]
    modified_row = lines[note_index + 4]
    assert modified_row.startswith("  modified: ")
    assert modified_row.removeprefix("  modified: ")
...
def test_narrow_terminal_preserves_git_markers(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0
    assert [line for line in result.stdout.splitlines() if tracked.name in line] == [f"`-- {tracked.name} [M]"]
