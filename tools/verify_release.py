...
def verify_release(snapshot: Mapping[str, bytes] | None = None) -> str:
    ...
    if re.fullmatch(r"\d{4}-\d{2}-\d{2}", runtime_version) is None:
        fail(f"release identity is not a canonical ISO date YYYY-MM-DD: {runtime_version}")
    try:
        datetime.date.fromisoformat(runtime_version)
    except ValueError:
        fail(f"release identity is not a real calendar date: {runtime_version}")
    ...
    if "install.sh | bash" not in readme_text:
        fail("README does not point users to the canonical installer command")
