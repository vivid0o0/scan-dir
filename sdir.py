...
# State subfile that records a user-selected persistent default config path.
# ``sdir --set-config <path>`` writes the resolved absolute path here; a later
# run reads it as the highest-precedence ``config.yaml`` location unless the
# user passes an explicit ``--config`` for that invocation.
DEFAULT_CONFIG_PATH_MARKER = "default-config-path"
...
# Hard floor on terminal columns. Below this the layout math breaks down, so
# terminal_columns reports the minimum readable layout width. The floor may
# make rows wider than the physical terminal, but it never shortens data.
TERMINAL_WIDTH_MINIMUM = 16
# Upper bound for layout geometry only. Lossless renderers may exceed this
# nominal width whenever accepted data itself is wider; the bound prevents
# hostile terminal metadata from manufacturing unbounded padding or rules.
...
def sanitize_terminal_text(value: object) -> str:
    ...
    for char in text:
        ...
        if category in {"Cc", "Cs"}:
            # Unicode control characters (Cc) and surrogates (Cs) are confined
            # to the BMP, so a four-digit escape covers the complete category.
...
def normalize_scan_data(value: object, key: str = "scan_data") -> frozenset[str]:
    ...
    The literal name ``max`` selects every data item, matching the config and
    CLI shorthand documented in the README (``scan-data: "max"`` / ``--scan-data
    max`` renders all available columns).
    """
    if not isinstance(value, str):
    ...
    for raw_item in value.split(","):
        ...
        if not item:
            continue
        if item == "max":
            # ``max`` means every data item; expand now so callers keep seeing
            # the resolved item set rather than a sentinel token.
            items.update(SCAN_DATA_ITEMS)
            continue
        if item not in SCAN_DATA_ITEMS:
            raise ConfigError(
                f"{key} contains unknown item '{sanitize_terminal_text(raw_item.strip())}'; "
                f"valid items: {', '.join(SCAN_DATA_ITEMS)} or 'max'"
            )
        items.add(item)
    if not items:
...
def user_state_dir() -> Path | None:
    """Resolve the owner-private state directory for persisted preferences.

    Mirrors the user-config resolution but for mutable runtime state, so a
    ``sdir --set-config`` marker never writes into the read-only managed app
    directory. ``SDIR_STATE_DIR`` wins, then XDG_STATE_HOME, then the
    platform default under ``~/.local/state``.
    """
    explicit_dir = os.environ.get("SDIR_STATE_DIR")
    if explicit_dir:
        candidate = expand_user_path(explicit_dir, "SDIR_STATE_DIR")
        if not candidate.is_absolute():
            raise ConfigError(f"SDIR_STATE_DIR must be absolute: {sanitize_terminal_text(explicit_dir)}")
        return candidate

    xdg_state = os.environ.get("XDG_STATE_HOME")
    if xdg_state:
        xdg_path = expand_user_path(xdg_state, "XDG_STATE_HOME")
        if xdg_path.is_absolute():
            return xdg_path / CONFIG_DIRECTORY_NAME

    try:
        home = Path.home()
    except RuntimeError:
        return None
    return home / ".local" / "state" / CONFIG_DIRECTORY_NAME


def default_config_marker_path() -> Path | None:
    state = user_state_dir()
    return state / DEFAULT_CONFIG_PATH_MARKER if state is not None else None


def persisted_default_config_path() -> Path | None:
    """Return the path recorded by ``--set-config``, if any.

    A malformed, relative, or unreadable marker is ignored rather than
    treated as an error: the runtime simply falls back to the standard
    resolution order.
    """
    marker = default_config_marker_path()
    if marker is None:
        return None
    try:
...
def set_default_config_path(value: str) -> Path:
    """Persist a new default user config path and return it as an absolute path."""
    expanded = expand_user_path(value, "config path")
    resolved = Path(os.path.abspath(expanded))
    marker = default_config_marker_path()
    if marker is None:
        raise ConfigError("no state directory is available to record the default config path")
    marker.parent.mkdir(parents=True, exist_ok=True)
    tmp = marker.with_name(f"{marker.name}.tmp")
    tmp.write_text(f"{resolved}\n", encoding="utf-8")
    os.replace(tmp, marker)
    return resolved


def user_config_path() -> Path | None:
    persisted = persisted_default_config_path()
...
def help_wrapped_lines(
    ...
    *codes: str,
    ...
    return [prefix + style(chunk, *codes, enabled=color) for chunk in wrap_cells(text, content_width)]
...
def help_table(title: str, rows: Sequence[tuple[str, str]], color: bool) -> list[str]:
    ...
    for option, description in rows:
        ...
        option_cells = cell_width(option)
        description_gap = max(1, option_width - option_cells)
        description_indent = 2 + option_cells + description_gap
        ...
        lines.append(f"  {option_text}{' ' * description_gap}{first_desc}")
        for chunk in description_lines[1:]:
            lines.append(" " * description_indent + style(chunk, ANSI_WHITE, enabled=color))
    return lines
    ...
    lines.extend(
        help_table(
            ...
            (
                ...
                (
                    '--scan-data <"item, item, ...">',
                    "Comma-separated items: tree, lines, size, modified, type, git, summary; max selects all. "
    ...
    lines.extend(
        help_table(
            ...
            (
                ...
                ("--set-config <path>", "Persist a new default config.yaml path (first token)."),
                (
...
def create_leaf(
    ...
    if kind == "link":
        ...
        attempts = 0
        while not state.timeout_reached():
            attempts += 1
            last_error = None
            ...
            try:
                ...
                if exc.errno in {errno.EACCES, errno.EPERM, errno.EMFILE, errno.ENFILE}:
                    break
            finally:
...
def terminal_columns() -> int:
    """Return a safe layout width without using terminal geometry as a data limit."""
    ...
    if raw_columns:
        ...
        for char in raw_columns:
            if not "0" <= char <= "9":
                break
            if columns > TERMINAL_WIDTH_MAXIMUM:
                continue
            digit = ord(char) - ord("0")
            if columns > (TERMINAL_WIDTH_MAXIMUM - digit) // 10:
                columns = TERMINAL_WIDTH_MAXIMUM + 1
            else:
...
def char_cell_width(char: str) -> int:
    ...
    others contribute 1. Centralised so cell_width and lossless wrapping
...
def wrap_cells(text: str, width: int) -> list[str]:
    if width <= 0 or cell_width(text) <= width:
        return [text]
    lines: list[str] = []
    start = 0
    while True:
        ...
        while end < len(text):
            char_width = char_cell_width(text[end])
...
def styled_columns(columns: Sequence[str], enabled: bool) -> str:
    ...
    for index, column in enumerate(columns):
        if index and cell_width(columns[index - 1]) >= METADATA_COLUMN_WIDTH:
            rendered.append(METADATA_SEPARATOR)
...
def render_padded_row(
    ...
    """Render a single tree row without shortening label or metadata data."""
    if marker:
        if node is None:
            styled_label = label
        else:
            styled_label = style(label, *row_style_for_node(node, color), enabled=color)
        styled_label = styled_label + " " + style_git_marker(marker, color)
        plain_label = label + f" {marker}"
    else:
        plain_label = label
        if header:
...
def render_tree_lines(
    ...
    lines: list[str] = []
    minimal_ascii = config.scan_styling == "minimal"
    marker = result.git_markers.get(".", "") if "git" in config.scan_data else ""
    _render_root_row(root, config, color, marker, lines)
    ...
    while pending:
        ...
        lines.extend(render_node_rows(plain_name, node, config, color, marker))
        if node.kind == "dir" and node.children:
            continuation = "|   " if minimal_ascii else "│   "
...
def run_clipboard_command(command: Sequence[str], text: str) -> tuple[bool, str | None, bool]:
    ...
    - ``environment_issue`` is True when the failure is an environment
      condition (connection refused, no display socket) rather than
...
def copy_to_clipboard(text: str) -> None:
    ...
    # Every candidate that ran failed due to environment issues (can't open
    # display, connection refused, stale socket). The desktop
...
def run(argv: Sequence[str]) -> int:
    try:
        ...
        if argv and is_status_token(argv[0]):
            ...
            return 0
        if argv and argv[0] == "--set-config":
            if len(argv) != 2:
                raise ConfigError("--set-config requires exactly one config path")
            resolved = set_default_config_path(argv[1])
            write_stream(sys.stdout, f"{PROGRAM_NAME}: default config path set to {resolved}\n")
            return 0
        ...
        if config.auto_copy:
            try:
                ...
                # The user explicitly requested clipboard delivery. Preserve
                # stdout, but report the requested copy as failed so the
                # README exit-code contract stays truthful.
